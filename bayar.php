...
?>
  ...
  <table width='478' border='1' class='table table-bordered table-striped'>
    ...
    while ($d = mysql_fetch_array($q)) {
      ...
      echo "
  <tr>
  <?php$tjhbs = $jhab + $d[lama];</?>
	<td>$no</td><td>$d[idsewa]</td><td>$d[tgl_pesan]</td><td>$lap[nm]</td><td>$tg</td><td>$d[lama] jam</td><td>$jmul</td><td>$tjhbs</td>"; ?><td><?php echo "Rp. " . number_format($d[harga]) . ",- ($d[jns])" ?></td>
      <td><?php echo "Rp. " . number_format($d[tot]) . ",-" ?></td><?php echo "<td>$bukti</td><td>$konfirmasi</td><td>$bayar $cetak</td>
  </tr>";
                                                                    $no++;
                                                                  }
                                                                    ?>
